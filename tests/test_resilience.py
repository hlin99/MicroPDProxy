...
from resilience import RetryConfig, RetryHandler, compute_backoff
...
class TestRetryExecute:
    ...
    async def test_no_retry_on_4xx(self):
        config = RetryConfig(
            enabled=True, max_retries=2, initial_backoff_ms=1
        )
        handler = RetryHandler(config)
...
"""
        ...
        args = types.SimpleNamespace(config=str(yaml_file), model=None, prefill=None,
                                     decode=None, port=8000,
