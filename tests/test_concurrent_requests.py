...
async def test_concurrent_chat_completions(client: AsyncClient):
    """Multiple concurrent non-streaming requests should all succeed."""
    payloads = [
        {**CHAT_PAYLOAD, "messages": [{"role": "user", "content": f"Hello {i}"}]}
        for i in range(5)
    ]

    tasks = [
        client.post("/v1/chat/completions", json=p)
        for p in payloads
    ]
...
async def test_concurrent_streaming(client: AsyncClient):
    """Multiple concurrent streaming requests should all produce valid SSE."""
