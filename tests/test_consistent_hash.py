...
class TestConsistentHashPolicy:
    ...
    def test_minimal_redistribution_on_node_removal(self):
        ...
        policy = ConsistentHashPolicy(workers=["w1", "w2", "w3", "w4"])
        before = {
            f"s{i}": policy.select(session_id=f"s{i}") for i in range(100)
        }
    ...
    def test_hash_key_priority(self):
        ...
        r1 = policy.select(
            header="sess-1", user=None, client_ip="1.2.3.4"
        )
        r2 = policy.select(
            header="sess-1", user="different", client_ip="5.6.7.8"
        )
    ...
    def test_add_worker(self):
        ...
        policy = ConsistentHashPolicy(workers=["w1", "w2"])
        before = {
            f"k{i}": policy.select(session_id=f"k{i}") for i in range(200)
        }
